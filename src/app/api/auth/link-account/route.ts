...
export async function POST(request: NextRequest) {
  try {
    ...
    const allowPasswordLinking =
      method === "password" && (!sessionUser || sessionUser.id === userId);
    ...
    if (method === "password") {
      ...
      const { data: currentUserData } =
        await supabaseAdmin.auth.admin.getUserById(authorizedUserId!);
      if (currentUserData?.user) {
        const currentAuthMethod =
          currentUserData.user.user_metadata?.auth_method || "password";
