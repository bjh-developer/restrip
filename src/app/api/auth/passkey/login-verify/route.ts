...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: userData, error: userError } =
      await supabaseAdmin.auth.admin.getUserById(storedCredential.user_id);
    ...
    // For autofill scenarios without email, the challenge should have been
    // stored with the user_id from the credential lookup
    if (!challengeData && storedCredential.user_id) {
      const { data } = await supabaseAdmin
        .from("webauthn_challenges")
        .select("*")
        .eq("user_id", storedCredential.user_id)
